...
describe('reaction cleanup subcommand', () => {
	...
	it('runs cleanup and replies with summary', async () => {
		...
		expect(interaction.reply).toHaveBeenCalledWith({ content: `Cleanup complete. Checked ${fakeResult.checked} mappings, removed ${fakeResult.removed}.`, flags: MessageFlags.Ephemeral });
	});
	...
	it('status returns last run info when present', async () => {
		...
		expect(interaction.reply).toHaveBeenCalledWith({ content: `Last cleanup: ${fakeDoc.lastRunAt.toISOString()} — checked ${fakeDoc.lastChecked}, removed ${fakeDoc.lastRemoved}.`, flags: MessageFlags.Ephemeral });
	});
});

...
import { ButtonStyle, MessageFlags, RoleResolvable, TextChannel } from 'discord.js';
		...
		try {
			...
			try {
				...
				if (logsChannelID) {
					...
					let logsChannel = guild?.channels.cache.get(logsChannelID as string) as TextChannel | undefined;
					if (!logsChannel) logsChannel = await guild?.channels.fetch(logsChannelID as string).catch(() => undefined) as TextChannel | undefined;

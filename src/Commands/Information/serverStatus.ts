	...
	run: async ({ interaction }) => {
		...
		try {
			if (subcommand === 'status') {
				...
				const server = services.find((s: { details: { game: string; }; }) => s.details.game.toLowerCase() === gameName?.toLowerCase());
				...
				const playerList = players.length > 0
					? players.map((p: { name: string; id: number; online: string; }) => `${p.name} (ID: ${p.id}, Online: ${p.online})`).join('\n')
					: 'No players online.';
				...
				const service = server.data.data.services.find((s: { details: { game: string; }; }) => s.details.game.toLowerCase() === gameName?.toLowerCase());
